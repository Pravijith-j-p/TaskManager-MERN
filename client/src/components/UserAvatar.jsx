...
import ChangePassword from "./ChangePassword"; // Adjust the path if necessary
...
const UserAvatar = () => {
  ...
  // Correct invocation of useLogoutMutation hook
  const [logoutUser] = useLogoutMutation();

  const logoutHandler = async () => {
    try {
      await logoutUser().unwrap(); // Unwrap the mutation result to handle errors
      dispatch(logout());
  ...
  return (
    ...
    <div>
      <Menu as="div" className="relative inline-block text-left">
        <div>
          <Menu.Button className="w-10 h-10 2xl:w-12 2xl:h-12 flex items-center justify-center rounded-full bg-blue-600 focus:outline-none">
